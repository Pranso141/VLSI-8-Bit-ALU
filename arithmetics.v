`timescale 1ns / 1ps
// 8 Bit CLA Adder modules and submodules required
module adder1b (
input a,b,cin,
output y,g,p,cout
);
...
//Adder with carry implemented in main directly

//8 Bit two's complementer
module twoscomp (
input [7:0] in1,
output wire [7:0] out,
output reg [4:0] flags
...
// 8 Bit Subtractor without Borrow
module sub(
input [7:0] in1,in2,
output reg [7:0] out,
output reg [4:0] flags
...
adderforinc inc (.in1(in1),.in2(8'b0000_0000),.cin(1'b1),.out(additionresult),.cout(flags[4])); // better than adding the number 1 as no propogation needs to occur 
...
subberfordec dec (.in1(in1),.in2(~(8'b0000_0000)),.cin(~(1'b1)),.out(subtractionresult),.cout(flags[4])); // better than adding the number 1 as no propogation needs to occur 
